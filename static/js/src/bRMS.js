...
function CreateStimulusContext(_id, stimulus, stimulus_vertical_flip, frameWidth, frameHeight, stimulus_width,
    ...
    let gap = 0;
    if (stimulus_side === 0) {
        gap = Math.max((frameWidth / 2) - (fixationWidth / 2) - stimulus_width, 0);
    } else if(stimulus_side === 2){
        gap = Math.max((frameHeight / 2) - (fixationWidth / 2) - stimulus_height, 0);
    }


    img.onload = function () {
        if (stimulus_side > 1) {
            stimulus_context.drawImage(img, 0, stimulus_location + gap,
                stimulus_width, stimulus_height);
        } else {
            stimulus_context.drawImage(img, stimulus_location + gap, 0,
                stimulus_width, stimulus_height);
